...
async function extractAndCheckLinks(content: string, postSlug: string): Promise<LinkCheckResult[]> {
  ...
  for (const link of links) {
    ...
    const statusColor = 
      statusStr === 'skipped' ? '\x1b[33m' :  // Yellow for skipped
      statusStr.startsWith('error') ? '\x1b[31m' : // Red for errors
      statusStr.startsWith('2') ? '\x1b[32m' : // Green for 2xx
      statusStr.startsWith('3') ? '\x1b[36m' : // Cyan for 3xx
      '\x1b[31m'; // Red for 4xx/5xx
      
    console.log(`${statusColor}${statusStr}\x1b[0m - ${link.type === 'image' ? '(Image)' : '(Link)'} ${link.url.substring(0, 70)}${link.url.length > 70 ? '...' : ''} [${postSlug}]`);
  }
...
async function main() {
  ...
  const resultsByPost = results.reduce((acc, result) => {
    if (!acc[result.post]) {
      acc[result.post] = [];
    }
  ...
  for (const [status, count] of Object.entries(statuses)) {
    const statusColor = 
      status === 'skipped' ? '\x1b[33m' :  // Yellow for skipped
      status.startsWith('error') ? '\x1b[31m' : // Red for errors
      status.startsWith('2') ? '\x1b[32m' : // Green for 2xx
      status.startsWith('3') ? '\x1b[36m' : // Cyan for 3xx
      '\x1b[31m'; // Red for 4xx/5xx
