...
export default function BlogCard({
  ...
  // Enhanced modern card variants with refined animations
  const cardVariants = {
    default:
    ...
    featured:
      'group relative flex flex-col lg:flex-row bg-gradient-to-br from-gray-950/95 via-gray-900/90 to-gray-950/95 rounded-2xl overflow-hidden shadow-xl hover:shadow-[0_35px_70px_-15px_rgba(0,0,0,0.8)] transition-all duration-500 ease-out border border-gray-800/50 hover:border-primary-500/50 backdrop-blur-xl transform hover:-translate-y-3 hover:scale-[1.01] will-change-transform col-span-full lg:min-h-[400px]',
    compact:
  ...
  const contentVariants = {
    default: 'relative flex flex-col p-6 flex-grow min-h-0',
    featured:
      'relative flex flex-col p-8 lg:p-10 flex-grow lg:w-3/5 xl:w-1/2 min-h-0',
    compact:
      'relative flex flex-col p-4 sm:p-5 flex-grow justify-between min-h-0'
  };
    ...
    <article className={cardVariants[variant]} data-variant={variant}>
      ...
      {post.coverImage && (
        ...
        >
          <div className={imageVariants[variant]}>
            ...
            {/* Improved reading time badge */}
            <div className="absolute top-3 right-3 flex items-center gap-2 px-3 py-1.5 bg-black/70 backdrop-blur-md rounded-xl text-xs text-white font-medium border border-white/10 shadow-lg">
            ...
            {showStats && (
              <button
                ...
                className="absolute top-3 left-3 p-2 bg-black/70 backdrop-blur-md rounded-xl text-white/80 hover:text-white border border-white/10 transition-all duration-200 hover:scale-110"
      ...
      <div className={contentVariants[variant]}>
        ...
        >
          ...
          >
            <div className="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-primary-400 to-purple-400" />
        ...
        {variant !== 'compact' && (
          <p
            className={`text-gray-300 leading-relaxed mb-5 line-clamp-2 group-hover:line-clamp-3 transition-all duration-300 ${
              variant === 'featured'
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {/* Stats (views, likes) - placeholder for future implementation */}
