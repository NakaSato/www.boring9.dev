...
import CopyCodeBlock from '../utils/CopyCodeBlock';
...
const MDXComponents = {
  ...
  a: (props: any) => (
    <Link {...props} className="text-blue-500 hover:text-blue-600 transition-colors duration-200">
      {props.children}
    </Link>
  ),
  pre: (props: any) => {
    ...
    if (codeElement && React.isValidElement(codeElement)) {
      ...
      if (React.isValidElement(elementProps.children)) {
        ...
        code = String(React.Children.toArray(elementProps.children)
      ...
      return <CopyCodeBlock code={code} language={language} className="my-6" />;
    }
    
    return <pre {...props} className="rounded-lg bg-gray-800 p-4 overflow-x-auto my-6" />;
  },
  code: (props: any) => {
